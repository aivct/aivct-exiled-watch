/*
	...
	TODO: Factor out pieces to a Battle.
 */
	...
	return {
		...
		*/
		...
		pieceThink: function(pieceID)
		{
			...
			// TODO: add ranged 
			let isRanged = Pieces.isPieceRangedByID(pieceID);
			
			let attackRange = Pieces.getPieceMaxRangeByID(pieceID);
			if(!isRanged) attackRange = 1;
			
			let validAttackTargetsID = Pieces.getValidTargetsID(pieceID, attackRange);
			if(validAttackTargetsID.length > 0)
			{
				...
				if(isRanged)
				{
					Abilities.abilityRangedAttackPiece(pieceID, targetID);
				}
				else 
				{
